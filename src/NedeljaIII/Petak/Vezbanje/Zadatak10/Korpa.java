...
public class Korpa {
   ...
   // Napisati odogarajuc(e) konstruktor(e) i metode za ovu klasu.
    ...
    public Korpa(VideoKlub vk, double ukupnaCena) {
     ...
     */
    void dodajFilmUKorpu(Film f, int n){
        for (FilmNaStanju film : vk.getKatalog() ){
            if(f.getIme() == film.getFilm().getIme()) {
                if(film.getKolicina() == 0){
                    System.out.println("Film " + f + " nema na stanju");
                } else if (n > film.getKolicina()) {
                    vk.iznajmiFilm(film.getFilm(), n);
                    korpa.add(film);
                    System.out.println("Nije moguce kupiti " + n + " filmova " + film.getFilm().getIme());
     ...
     */
    void izbaciIzKorpe(Film f){
        for (FilmNaStanju film : vk.getKatalog() ){
            if(f.getIme() == film.getFilm().getIme()) {
                if(film.getKolicina() >= 0){
                    vk.dodajFilm(film.getFilm(), film.getCena());
                    korpa.remove(film);
                    System.out.println("Izbacili ste kopiju filma " + film.getFilm().getIme() + " iz korpe");
                }
                break;
            }
        }
     ...
     */
    void izbaciIzKorpe(Film f, int n){
        for (FilmNaStanju film : vk.getKatalog() ){
            if(f.getIme() == film.getFilm().getIme()) {
                if(film.getKolicina() > n){
                    vk.dodajFilm(film.getFilm(), film.getCena(), n);
                    korpa.remove(film);
                    System.out.println("Izbacili ste " + n + " kopija filma " + film.getFilm().getIme() + " iz korpe");
                }
                else if (n >= film.getKolicina()) {
                    vk.dodajFilm(film.getFilm(), film.getCena(), n);
                    korpa.remove(film);
                    System.out.println("Izbacili ste sve kopije filma " + film.getFilm().getIme() + " iz korpe");
                }
                break;
            }
        }
	...
	"Izbacili ste sve kopije filma {nazivFilma} iz korpe."
     */
    void izbaciIzKorpeSveKopije(Film f) {
        ArrayList<FilmNaStanju> izbacitiIzKorpe = new ArrayList<>();
        for (FilmNaStanju film : korpa){
            if(f.getIme() == film.getFilm().getIme()){
                izbacitiIzKorpe.add(film);
            }
        }
        for (FilmNaStanju film : izbacitiIzKorpe){
            korpa.remove(film);
            vk.dodajFilm(film.getFilm(), film.getCena(), film.getKolicina());
            System.out.println("Izbacili ste sve kopije filma " + film.getFilm().getIme() + " iz korpe");
        }
    }
